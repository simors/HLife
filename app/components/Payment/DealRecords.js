/**
 * Created by yangyang on 2017/5/17.
 */
...
import {getUserDealRecords} from '../../action/paymentActions'
import {formatLeancloudTime} from '../../util/numberUtils'
import {selectPaymentDealRecords} from '../../selector/paymentSelector'
...
class DealRecords extends Component {
  ...
  renderRow(rowData) {
    ...
    if (deal.dealType == 1) {
    ...
    } else if (deal.dealType == 4) {
      return (
        <View style={styles.recordItemView}>
          <View style={styles.itemView}>
            <View>
              <Text style={styles.earnTitle}>话题打赏收益</Text>
            </View>
            <View>
              <Text style={styles.earnValue}>+{deal.cost}</Text>
            </View>
      ...
      return (
        <View style={styles.recordItemView}>
          <View style={styles.itemView}>
            <View>
              <Text style={styles.earnTitle}>提现</Text>
            </View>
            <View>
              <Text style={[styles.earnValue, {color: 'green'}]}>-{deal.cost}</Text>
            </View>
          </View>
          <View style={[styles.itemView, {paddingTop: normalizeH(15)}]}>
            <View style={{flexDirection: 'row', alignItems: 'center'}}>
...
const mapStateToProps = (state, ownProps) => {
  ...
  let records = selectPaymentDealRecords(state, ownProps.userId)
...
export default connect(mapStateToProps, mapDispatchToProps)(DealRecords)
